...
- Wazuh 4.x with Elasticsearch backend
...
Wazuh stores alerts as JSON documents in Elasticsearch. A typical alert looks like:
...
The `description` field is the primary input to the LLM. A vague description produces a vague classification. Include specifics:
...
MIN_LEVEL   = int(os.environ.get("MIN_RULE_LEVEL",    "7"))   # Wazuh level 7+ = medium/high
...
def build_description(alert: dict) -> str:
    ...
    if src_ip:
        parts.append(f"from source IP {src_ip}")
    if full_log:
        parts.append(f"Log: {full_log[:300]}")
...
def build_soc_alert(alert: dict) -> dict:
    ...
    if rule.get("groups"):
        context_parts.append(f"Groups: {', '.join(groups)}")
...
def write_enrichment(alert_id: str, wazuh_alert: dict, analysis: dict) -> None:
    doc = {
        ...
        "wazuh_rule_id":         wazuh_alert.get("rule", {}).get("id"),
        "wazuh_rule_description":wazuh_alert.get("rule", {}).get("description"),
        "wazuh_rule_level":      wazuh_alert.get("rule", {}).get("level"),
...
def get_alerts_since(since: str) -> list[dict]:
    """Query Elasticsearch for Wazuh alerts above minimum level since `since`."""
    query = {
        "bool": {
            "must": [
...
def main():
    ...
    # Start from 5 minutes ago to avoid re-processing existing alerts on first run
    since = (datetime.datetime.utcnow() - datetime.timedelta(minutes=5)).isoformat() + "Z"

    while True:
        try:
            hits = get_alerts_since(since)
            print(f"[{since}] Found {len(hits)} new alerts (level >= {MIN_LEVEL})")
            ...
            for hit in hits:
                ...
                # Avoid hammering the engine — 1 analysis per 2 seconds minimum
                time.sleep(2)

            # Advance the polling window
            if hits:
...
|---|---|
| `fallback_used: true` | Engine was unreachable or model output was invalid. Do not auto-escalate on this result — flag for manual review. |
...
| `risk_score` | 0–100. Use as a secondary triage signal alongside Wazuh rule level. |
| `false_positive_likelihood` | 0–1. High value (>0.6) = strong signal this is benign noise. |
| `reason` | Plain-language explanation — most useful for analyst notes. |
...
- Timeout too short: Ollama on CPU can take 60+ seconds. Increase `LOCAL_LLM_ENGINE_TIMEOUT_MS` in `soc-backend/.env.local` to `120000` (120s)

**`elasticsearch.exceptions.NotFoundError` (index not found):**
- Your Wazuh index name might differ: check with `curl http://localhost:9200/_cat/indices | grep wazuh`
- Update the `get_alerts_since()` query to match your actual index name
